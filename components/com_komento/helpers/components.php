	...
	*/
	public function getAvailableComponents()
	{
		...
		if( empty($components) )
		{
			...
			$folders	= JFolder::folders( JPATH_ROOT . DS . 'components', 'com_', false, false, array('.svn', 'CVS', '.DS_Store', '__MACOSX', 'com_komento') );

			foreach ($folders as $folder)
			{
				if( JFile::exists( JPATH_ROOT . DS . 'components'  . DS . $folder . DS . 'komento_plugin.php' ) )
				{
			...
			foreach ($folders as $folder)
			{
				if( JFile::exists( KOMENTO_ROOT . DS . 'komento_plugins' . DS . $folder . '.php' ) )
				{
					$components[$folder]	= $folder;
				}
			}
	...
	/**
	 * Gets an object of the component type.
	 *
	 * @access	public
	 * @param	string	$name	The title of the component.
	 * @return	obj
	 */
	public function getComponentObject( $name )
	{
		return Komento::loadApplication( $name );
	}
	...
	public static function isInstalled( $optionName )
	{
		...
		if( $componentName
			&& JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.$optionName.DS.'admin.'.$componentName.'.php')
			|| JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.$optionName.DS.$componentName.'.php')
			)
		{
			return true;
		}
