...
if( $attachments !== false && count( $attachments ) > 0 ) { ?>
	<div class="kmt-attachments attachmentWrap kmt-view-<?php echo $system->config->get( 'attachment_layout' ); ?>">
	...
	<?php if( $system->my->allow( 'download_attachment' ) ) { ?>
		<ul class="kmt-attachments-list clearfix attachmentList">
			<?php foreach( $attachments as $attachment ) { ?>
				<li class="kmt-attachment-item attachmentFile file-<?php echo $attachment->id; ?>" attachmentid="<?php echo $attachment->id; ?>" attachmentname="<?php echo $attachment->filename; ?>">
					<?php if( $system->my->allow( 'delete_attachment' ) ) { ?>
						<a href="javascript:void(0);" class="attachmentDelete"><i class="kmt-attachment-delete"></i></a>
					<?php } ?>
					<a href="<?php echo $attachment->link; ?>" class="kmt-attachment-file"><i class="icon-mime icon-mime-<?php echo $attachment->class; ?>"></i><span><?php echo $attachment->filename; ?></span></a>
				</li>
			<?php } ?>
		</ul>
	<?php } else { ?>
