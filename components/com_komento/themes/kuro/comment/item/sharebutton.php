...
if( $system->config->get( 'enable_share' ) && $system->my->allow( 'share_comment' ) ) { ?>
	<?php
		...
		$temp = $row->comment;
		$temp = KomentoCommentHelper::parseBBCode( $temp );
		$temp = nl2br( $temp );
		$temp = strip_tags( $temp );
		$title = substr( $temp, 0, 30 );
	...
	<span class="kmt-share-wrap">
		<!-- Share button -->
		<a class="shareButton kmt-btn kmt-share" href="javascript:void(0);"><i></i><span><?php echo JText::_( 'COM_KOMENTO_COMMENT_SHARE' ); ?></span></a>
