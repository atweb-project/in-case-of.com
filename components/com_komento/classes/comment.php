...
class KomentoComment extends JObject
{
	...
	public function save()
	{
		...
		if( $config->get( 'notification_enable' ) )
		{
			...
			{
				Komento::getHelper( 'Notification' )->push( 'new', 'subscribers,admins,author', array( 'commentId' => $this->id ) );
			}

			if( $commentTable->published == 2 && $config->get( 'notification_event_new_pending' ) )
			{
				Komento::getHelper( 'Notification' )->push( 'moderate', 'admins,author', array( 'commentId' => $this->id ) );
			}
		}
	...
	public function publish( $type = '1' )
	{
		...
		{
			// send email
			Komento::getHelper( 'Notification' )->push( 'new', 'subscribers,admins,author', array( 'commentId' => $commentTable->id ) );
