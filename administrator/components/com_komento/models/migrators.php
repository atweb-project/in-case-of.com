...
class KomentoModelMigrators extends JModel
{
	public $component;
	public $cid;
...
class KomentoModelMigratorJooComments extends KomentoModelMigrators
{
	// did not make the final cut due to no user id column in joo comments
	...
	function save( $comment )
	{
		...
		$new->created		= $comment->publish_date;
	...
	function load( $id )
	{
		$query = 'SELECT * FROM `#__joocomments` WHERE `id` = ' . $this->_db->quote( $id );
		$this->_db->setQuery( $query );
		return $this->_db->loadObject();
	}
	...
	{
		$query  = 'SELECT * FROM `#__joocomments`';
	...
	{
		$query = 'SELECT COUNT(1) FROM `#__joocomments`';
...
class KomentoModelMigratorJAComment extends KomentoModelMigrators
{
	...
	function getUniqueComponents()
	{
		...
		$query = 'SELECT DISTINCT `option` FROM `#__jacomment_items` ORDER BY `option`';
		$this->_db->setQuery( $query );
		return $this->_db->loadResultArray();
	}
