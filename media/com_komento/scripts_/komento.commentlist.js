...
Komento.require()
	.library('effects/highlight', 'effects/fade')
		...
		Komento.Controller(
			...
			{ return {
				...
				addComment: function(nodeId, html, contentLink) {
					...
					if(self.noComment().length != 0) {
						...
						if(Komento.options.config.enable_threaded == 1) {
							if(nodeId == 0) {
								if(Komento.options.config.load_previous == 1 || self.loadMore().length == 0 || self.loadMore().is(':hidden')) {
									self.commentList().append(commentHtml);
									implement = 1;
								}
							} else {
								// if is in reply mode, append comment after the form
				...
				addChildCount: function(parentid) {
					...
					if($('.' + parentid).exists() && parseInt($('.' + parentid).attr('parentid').split('-')[1]) > 0) {
