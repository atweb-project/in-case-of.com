...
Komento.Controller(
	...
	{ return {
		...
		"{tncRead} click": function() {
			Komento.require().view('dialogs/comment.tnc').done(function() {
				$.dialog({
					title: $.language( 'COM_KOMENTO_FORM_TNC' ),
					customClass: 'kmt-dialog',
					width: 500,
					showOverlay: false,
					content: Komento.View('dialogs/comment.tnc')
				});
			});
		},
		...
		postComment: function() {
			...
			},
			{
				...
				fail: function(data) {
					self.errorNotification($.language('COM_KOMENTO_ERROR'));
				...
				error: function(message) {
					self.errorNotification(message);
		...
		staticReply: function(parentid) {
			...
			self.element.find('h3').html('').append(title).append(parentContainer);
			self.submitButton().text($.language('COM_KOMENTO_FORM_REPLY'));
		...
		cancelStaticReply: function() {
			self.parentid = 0;
			self.element.find('h3').text($.language('COM_KOMENTO_FORM_LEAVE_YOUR_COMMENTS'));
		},
		...
		reply: function(item) {
			...
			self.element.find('h3').text($.language('COM_KOMENTO_FORM_REPLY'));
