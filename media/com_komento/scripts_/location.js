	...
	function(self) { return {
		...
		addInfoWindow: function(marker, info) {
			...
			google.maps.event.addListener(infoWindow, 'domready', function() {
				eblog.ratings.setup( 'ebpostmap_' + info.id + '-ratings' , true , 'entry' );
				$('#ebpostmap_' + info.id + '-ratings').removeClass('ui-state-disabled');
				$('#ebpostmap_' + info.id + '-ratings-form').find('.blog-rating-text').hide();
			});
		}
	}}
);
